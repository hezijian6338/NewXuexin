...
import com.zhbit.xuexin.student.service.GrantScholarshipService;
 ...
 * @version 1.0
 */
@Service("grantScholarshipService")
@Transactional(readOnly = true)
public class GrantScholarshipServiceImpl implements GrantScholarshipService {
    private Logger logger = LoggerFactory.getLogger(GrantScholarshipServiceImpl.class);

    @Autowired
    @Qualifier("grantScholarshipDao")
    private GrantScholarshipDao dao;
    ...
    public int save(GrantScholarship info, String userId, String reportdateStr) {
        if (info != null) {
            ...
            if(existResult == 0){
	            ...
	            try {
	                info.setReportdate(df.parse(reportdateStr));
	            }
    ...
    public GrantScholarship getGrantScholarshipByid(String id) {
        return dao.getGrantScholarshipByid(id);
    }
    ...
    @Override
    @Transactional(readOnly = false)
    public int update(GrantScholarship info, GrantScholarship oldInfo, String reportdateStr) {
        if (info != null) {
            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
            Students stu = studentsDao.getStudentByNo(info.getStudentno());
			if (stu == null) {
    ...
    public void delete(String ids) {
        if (ids != null || "".equals(ids)) {
            ...
            for (String id : sids) {
                GrantScholarship info = dao.getGrantScholarshipByid(id);
                dao.delete(info);
            }
        }
    }
                ...
                try {
                    ...
                    if (row.getCell(0) != null && !"".equals(row.getCell(0).getStringCellValue())) {
                        ...
                        if (stu == null) {// 存在学生信息才导入
                            ...
                            info.setMajor(row.getCell(3).getStringCellValue());
                            String sex = "0";
                            if (row.getCell(4) != null && "女".equals(row.getCell(4).getStringCellValue())) {
                                sex = "1";
                            }
                            info.setSex(sex);
                            info.setNation(row.getCell(5).getStringCellValue());
                            info.setPoliticalstatus(row.getCell(6).getStringCellValue());
                            info.setAcceptancedate(row.getCell(7).getStringCellValue());
                            info.setFamilyeconomic(row.getCell(8).getStringCellValue());
                            ...
                            if (row.getCell(11) != null && !row.getCell(11).getStringCellValue().equals("")) {
                                info.setReportdate(df.parse(row.getCell(11).getStringCellValue()));
                            }
                            if (row.getCell(12) != null && !row.getCell(12).getStringCellValue().equals("")) {
                                info.setMemo(row.getCell(12).getStringCellValue());
                            }
                            info.setCreateTime(new Date());
                            info.setStuId(stu.getStuId());
                            int existResult = getDateExist(info);
     ...
     */
    @Override
    public ExportExcelVO exportExcelList(Page<GrantScholarship> page) {
        page.setPage(1);
        page.setRows(100000);
        // 获取查询结果数据集
        Page<GrantScholarship> pageResult = dao.getList(page);
        List<GrantScholarship> list = pageResult.getResult();
        // 设置表头
        String[] title = { "学号", "姓名", "学院名称", "专业", "性别", "民族", "政治面貌", "入学年月", "家庭经济困难程度", "学年", "学期", "制表日期", "备注" };
        // 设置文件名
        String filename = "助学金表";
        // 设置内容
        ExportExcelVO vo = new ExportExcelVO();
        List<Object[]> listInfo = new ArrayList<Object[]>();
        for (int i = 0; i < list.size(); i++) {
            GrantScholarship info = list.get(i);
            String[] str = new String[13];
            str[0] = info.getStudentno();
            str[1] = info.getStuname();
            str[2] = info.getOrgName() == null ? "" : info.getOrgName();
            str[3] = info.getMajor() == null ? "" : info.getMajor();
            String sex = "男";
            if ("1".equals(info.getSex())) {
                sex = "女";
            }
            str[4] = sex;
            str[5] = info.getNation() == null ? "" : info.getNation();
            str[6] = info.getPoliticalstatus() == null ? "" : info.getPoliticalstatus();
            str[7] = info.getAcceptancedate() == null ? "" : info.getAcceptancedate();
            str[8] = info.getFamilyeconomic() == null ? "" : info.getFamilyeconomic();
            str[9] = info.getAcademicyear() == null ? "" : info.getAcademicyear();
            str[10] = info.getTerm() == null ? "" : info.getTerm();
            SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
            str[11] = info.getReportdate() == null ? "" : df.format(info.getReportdate());
            str[12] = info.getMemo() == null ? "" : info.getMemo();
            listInfo.add(str);
        }
