...
@Scope("prototype")
public class SubjectContestAction extends BaseAction implements ModelDriven<SubjectContest> {
    ...
    private SubjectContest info;

    @Override
    public SubjectContest getModel() {
        if (info == null) {
            info = new SubjectContest();
        }
        return info;
    }
    ...
    @Resource
    private SubjectContestService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<SubjectContest> page = service.getList(new Page<SubjectContest>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(), rewarddateStr);
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            SubjectContest oldInfo = service.getSubjectContestByid(info.getId());
            service.update(info, oldInfo, rewarddateStr);
            //
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    /**
     * @Title: getSubjectContest
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getSubjectContest() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            SubjectContest info = service.getSubjectContestByid(sid);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生学科竞赛获奖表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "SubjectContest.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<SubjectContest>(req));
     ...
     * @return void
     ...
     public void uploadAttachment() {
         try {
             if (attachment != null) {
                 service.uploadFile(sid, attachment, attachmentFileName);
             }
             StringBuffer result = new StringBuffer();
             result.append("<script type=\"text/javascript\">");
     ...
     public void downloadFile() {
         ...
         try {
             SubjectContest info = service.getSubjectContestByid(sid);
             String suffix = info.getDocpath().substring(info.getDocpath().indexOf("_"));
             getResponse().setContentType("application/octet-stream; charset=utf-8");
             getResponse().setHeader("Content-Disposition",
                     "attachment;filename=" + new String((info.getStudentno()+"学科竞赛附件" + suffix).getBytes("GBK"), "ISO8859-1"));

             String templatePath = Const.student_contest + info.getDocpath();
             File temFile = new File(templatePath);
             byte[] bytes = new byte[0xffff];
             in = new FileInputStream(temFile);
             ServletOutputStream out = getResponse().getOutputStream();
    ...
    public SubjectContest getInfo() {
        return info;
    }

    public void setInfo(SubjectContest info) {
        this.info = info;
    }
