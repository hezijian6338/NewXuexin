 ...
 * @date 创建时间：2016-3-18 上午2:21:37
 * @version 1.0
 */
@Controller("learningGuidInfoAction")
@Scope("prototype")
public class LearningGuidInfoAction extends BaseAction implements ModelDriven<LearningGuidInfo> {

    /**
     * @Fields serialVersionUID : TODO(简单说明是做什么的。)
     */
    private static final long serialVersionUID = -4870713314572209952L;

    private LearningGuidInfo info;

    @Override
    public LearningGuidInfo getModel() {
        if (info == null) {
            info = new LearningGuidInfo();
        }
        return info;
    }
    ...
    @Resource
    private LearningGuidInfoService service;
    private LearningGuidStudentsListService service2;
     ...
     *@date 创建时间：2017年4月10日 下午10:07:32
     ...
     * 
     *@author 许彩开 email:1121836563@qq.com
     *@date 创建时间：2017年4月10日 下午10:08:47
     *@Description:TODO()
     *@return
     */
     ...
     * 
     *@author 许彩开 email:1121836563@qq.com
     *@date 创建时间：2017年4月10日 下午10:15:25
     *@Description:TODO()
     *@return
     */
    ...
    public void getList() {
        ...
        try {
            HttpServletRequest req = getRequest();
            Page<LearningGuidInfo> page = service.getList(new Page<LearningGuidInfo>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(), guiddateStr);
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            LearningGuidInfo oldInfo = service.getLearningGuidInfoByid(info.getId());
            int state = service.update(info, oldInfo, guiddateStr);
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    /**
     * @Title: getLearningGuidInfo
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getLearningGuidInfo() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            LearningGuidInfo info = service.getLearningGuidInfoByid(lgId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("导学内容表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "LearningGuidInfo.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<LearningGuidInfo>(req));
    ...
    public LearningGuidInfo getInfo() {
        return info;
    }

    public void setInfo(LearningGuidInfo info) {
        this.info = info;
    }
