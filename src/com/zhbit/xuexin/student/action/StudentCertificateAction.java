 ...
 * @version 1.0
 */
@Controller("studentCertificateAction")
@Scope("prototype")
public class StudentCertificateAction extends BaseAction implements ModelDriven<StudentCertificate> {
    ...
    private StudentCertificate info;

    @Override
    public StudentCertificate getModel() {
        if (info == null) {
            info = new StudentCertificate();
        }
        return info;
    }
    ...
    public void getList() {
        ...
        try {
            ...
            Page<StudentCertificate> page = service.getList(new Page<StudentCertificate>(req));
            //
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            StudentCertificate oldInfo = service.getStudentCertificateByid(info.getId());
    ...
    public void getStudentCertificate() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            StudentCertificate info = service.getStudentCertificateByid(sid);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生毕业相关证书表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "StudentCertificate.xls";
    ...
    * @return void
    ...
    public void uploadAttachment() {
        try {
            if (attachment != null) {
                ...
                service.uploadFile(sid, attachment, suffix);
            }
            StringBuffer result = new StringBuffer();
            result.append("<script type=\"text/javascript\">");
    ...
    public void downloadFile() {
        ...
        try {
            ...
            getResponse().setContentType("application/octet-stream; charset=utf-8");
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String(("学生毕业相关证书表附件" + suffix).getBytes("GBK"), "ISO8859-1"));

            String templatePath = Const.student_certificate + info.getDocpath();
            File temFile = new File(templatePath);
            byte[] bytes = new byte[0xffff];
            in = new FileInputStream(temFile);
            ServletOutputStream out = getResponse().getOutputStream();
    ...
    public StudentCertificate getInfo() {
        return info;
    }

    public void setInfo(StudentCertificate info) {
        this.info = info;
    }
