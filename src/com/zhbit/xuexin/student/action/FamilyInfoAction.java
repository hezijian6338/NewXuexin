 ...
 * @date 创建时间：2016-3-10 下午1:17:27
 * @version 1.0
 */
@Controller("familyInfoAction")
@Scope("prototype")
public class FamilyInfoAction extends BaseAction {

	/**
	 * @Fields serialVersionUID : TODO(简单说明是做什么的。)
	 */
	private static final long serialVersionUID = -5670020230685597126L;
	...
	public void getList() {
		...
		try {
			...
			Page<FamilyInfo> page = service.getList(new Page<FamilyInfo>(req));
			//
	...
	public void getSelf() {
		...
		try {
			...
			Page<FamilyInfo> page = service.getSelf(new Page<FamilyInfo>(req),
					stu);
			//
			map.put("total", page.getCount());
			map.put("rows", page.getResult());
			...
			if (excel != null) {
				...
				if (".xlsx".equalsIgnoreCase(suffix)
					...
					User user = (User) getSession().getAttribute(
							Const.SESSION_USER);
					count = service.importExcel(excel, user, suffix);
					if (count[0] < 0) {
	...
	public void exportTemplate() {
		...
		try {
			...
			getResponse().setHeader(
					"Content-Disposition",
					"attachment;filename="
							+ new String("家庭信息导入模板.xls".getBytes("GBK"),
									"ISO8859-1"));
	...
	public void save() {
		Map<String, Object> map = Const.getJsonMap();
		try {
			int state = service.save(studentno, stuname, call, name,
					politicalstatus, jobduty, company, companyaddress, telno,
					postcode, getSessionUser().getUserId());
			//
			if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else {
	...
	public void saveFamilyInfo() {
		Map<String, Object> map = Const.getJsonMap();
		try {
			Students students = (Students) getSession().getAttribute(
					Const.SESSION_STUDENT);
			int state = service.save(students.getStudentno(),
					students.getStuname(), call, name, politicalstatus,
					jobduty, company, companyaddress, telno, postcode,
	...
	public void update() {
		Map<String, Object> map = Const.getJsonMap();
		try {
			service.update(id, studentno, stuname, call, name, politicalstatus,
					jobduty, company, companyaddress, telno, postcode);
			//
			map.put("status", Const.CODE_SUCCESS);
		} catch (Exception err) {
	...
	public void updateFamilyInfo() {
		...
		try {
			Students students = (Students) getSession().getAttribute(
					Const.SESSION_STUDENT);
			FamilyInfo temp = service.getFamilyInfo(id);
			if (null != temp
					&& students.getStudentno().equals(temp.getStudentno())) {
				service.update(id, students.getStudentno(),
						students.getStuname(), call, name, politicalstatus,
						jobduty, company, companyaddress, telno, postcode);
				map.put("status", Const.CODE_SUCCESS);
			} else {
	...
	public void deleteSelfFamilyInfo() {
		Map<String, Object> map = Const.getJsonMap();
		try {
			Students students = (Students) getSession().getAttribute(
					Const.SESSION_STUDENT);
			if (ids != null || "".equals(ids)) {
				String[] sidStrings = ids.split(",");
				for (String id : sidStrings) {
					FamilyInfo temp = service.getFamilyInfo(id);
					if (null != temp
							&& students.getStudentno().equals(
									temp.getStudentno())) {
						service.deleteFamilyInfo(temp);
					}
				}
			}
	...
	public void getFamilyInfo() {
		Map<String, Object> map = Const.getJsonMap();
		try {
			FamilyInfo info = service.getFamilyInfo(id);
			map.put("info", info);
			map.put("status", Const.CODE_SUCCESS);
		} catch (Exception err) {
			err.printStackTrace();
	...
	public void getFamilyInfoSelf() {
		...
		try {
			...
			FamilyInfo info = service.getFamilyInfo(id);
			if (info != null
					&& students.getStudentno().equals(info.getStudentno())) {
				map.put("info", info);
				map.put("status", Const.CODE_SUCCESS);
			} else {
				map.put("status", Const.CODE_FAIL);
