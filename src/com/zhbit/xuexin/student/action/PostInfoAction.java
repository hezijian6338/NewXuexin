...
@Scope("prototype")
public class PostInfoAction extends BaseAction implements ModelDriven<PostInfo> {
    ...
    */ 
    ...
    public void getList() {
        ...
        try {
            ...
            Page<PostInfo> page = service.getList(new Page<PostInfo>(req));
            //
    ...
    public void save() {
        ...
        try {
        	HttpServletRequest req = getRequest();
            int state = service.save(info, getSessionUser().getUserId(), new Page<PostInfo>(req));
            //
            if(state==1){
                map.put("status", Const.CODE_SUCCESS);
            }else{
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            PostInfo oldInfo = service.getPostInfoByid(info.getId());
    ...
    public void getPostInfo() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            PostInfo info = service.getPostInfoByid(pId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
        ...
        try {
            ...
            if (excel != null) {
                ...
                if (".xlsx".equalsIgnoreCase(suffix) || ".xls".equalsIgnoreCase(suffix)) {
                    ...
                    count = service.importFile(excel, user, suffix, new Page<PostInfo>(req));
                    if (count[0] < 0) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生档案信息导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "PostInfo.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<PostInfo>(req));
