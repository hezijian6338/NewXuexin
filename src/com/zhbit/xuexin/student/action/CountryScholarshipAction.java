 ...
 *@date 创建时间：2016-3-19 下午9:36:45
 *@version 1.0
 */
@Controller("countryScholarshipAction")
@Scope("prototype")
public class CountryScholarshipAction extends BaseAction implements ModelDriven<CountryScholarship> {

    /** 
    * @Fields serialVersionUID : TODO(简单说明是做什么的。) 
    */ 
    private static final long serialVersionUID = -4586549584877687806L;

    private CountryScholarship info;
    @Override
    public CountryScholarship getModel() {
        if (info == null) {
            info = new CountryScholarship();
        }
        return info;
    }
    ...
    @Resource
    private CountryScholarshipService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<CountryScholarship> page = service.getList(new Page<CountryScholarship>(req));
            //
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            CountryScholarship oldInfo = service.getCountryScholarshipByid(info.getId());
    ...
    /**
     * @Title: getCountryScholarship
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getCountryScholarship() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            CountryScholarship info = service.getCountryScholarshipByid(csId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void importFile() {
        try {
            ...
            result.append("var isNotNull=").append(isNotNull).append(";");
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("国家光大奖学金导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "CountryScholarship.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<CountryScholarship>(req));
    ...
    public CountryScholarship getInfo() {
        return info;
    }

    public void setInfo(CountryScholarship info) {
        this.info = info;
    }
