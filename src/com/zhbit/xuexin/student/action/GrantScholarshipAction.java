...
@Scope("prototype")
public class GrantScholarshipAction extends BaseAction implements ModelDriven<GrantScholarship> {
    ...
    */ 
    ...
    private GrantScholarship info;
    @Override
    public GrantScholarship getModel() {
        if (info == null) {
            info = new GrantScholarship();
        }
        return info;
    }
    ...
    @Resource
    private GrantScholarshipService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<GrantScholarship> page = service.getList(new Page<GrantScholarship>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(),reportdateStr);
            if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            GrantScholarship oldInfo = service.getGrantScholarshipByid(info.getId());
            int state =service.update(info,oldInfo,reportdateStr);
            if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
				map.put("status", "SnoOrSnameIsError");
    ...
    /**
     * @Title: getGrantScholarship
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getGrantScholarship() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            GrantScholarship info = service.getGrantScholarshipByid(gsId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("助学金表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "GrantScholarship.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<GrantScholarship>(req));
    ...
    public GrantScholarship getInfo() {
        return info;
    }

    public void setInfo(GrantScholarship info) {
        this.info = info;
    }
