 ...
 * @version 1.0
 */
@Controller("trainInfoMasterAction")
@Scope("prototype")
public class TrainInfoMasterAction extends BaseAction implements ModelDriven<TrainInfoMaster> {
    ...
    private TrainInfoMaster info;

    @Override
    public TrainInfoMaster getModel() {
        if (info == null) {
            info = new TrainInfoMaster();
        }
        return info;
    }

    private String loadId;//加载历史数据时传回的id
    
    private String trainsDateStr;
    ...
    @Resource
    private TrainInfoMasterService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<TrainInfoMaster> page = service.getList(new Page<TrainInfoMaster>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(),trainsDateStr);
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            TrainInfoMaster oldInfo = service.getTrainInfoMasterByid(info.getId());
            service.update(info, oldInfo,trainsDateStr);
            //
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    /**
     * @Title: getTrainInfoMaster
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getTrainInfoMaster() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            TrainInfoMaster info = service.getTrainInfoMasterByid(loadId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生培训情况主表导入模板.xlsx".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "TrainInfoMaster.xlsx";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<TrainInfoMaster>(req));
    ...
    public TrainInfoMaster getInfo() {
        return info;
    }

    public void setInfo(TrainInfoMaster info) {
        this.info = info;
    }
    ...
    public String getTrainsDateStr() {
        return trainsDateStr;
    }

    public void setTrainsDateStr(String trainsDateStr) {
        this.trainsDateStr = trainsDateStr;
    }
