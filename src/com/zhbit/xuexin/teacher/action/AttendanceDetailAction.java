 ...
 * @version 1.0
 */
@Controller("attendanceDetailAction")
@Scope("prototype")
public class AttendanceDetailAction extends BaseAction implements ModelDriven<AttendanceDetail> {
    ...
    private AttendanceDetail info;

    @Override
    public AttendanceDetail getModel() {
        if (info == null) {
            info = new AttendanceDetail();
        }
        return info;
    }
    ...
    public void getList() {
        ...
        try {
            ...
            Page<AttendanceDetail> page = service.getList(new Page<AttendanceDetail>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(), attendanceTimeStr);
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            AttendanceDetail oldInfo = service.getAttendanceDetailByid(info.getId());
            service.update(info, oldInfo, attendanceTimeStr);
            //
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void getAttendanceDetail() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            AttendanceDetail info = service.getAttendanceDetailByid(sid);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "AttendanceDetail.xls";
    ...
    public void getDetailList() {
        ...
        try {
            HttpServletRequest req = getRequest();
            List<AttendanceDetailVO> list = service.getDetailList(new Page<AttendanceDetail>(req).getParas().get("selectedcourseno").toString());
            //
    ...
    public void getDete() {
        ...
        try {
            HttpServletRequest req = getRequest();
            List<String> list = service.getAttendanceTimes(new Page<AttendanceDetail>(req).getParas().get("selectedcourseno").toString());
            //
            map.put("result", list);
            map.put("status", Const.CODE_SUCCESS);
        }
    ...
    public AttendanceDetail getInfo() {
        return info;
    }

    public void setInfo(AttendanceDetail info) {
        this.info = info;
    }
	...
	/**
	 * @param selectedcourseno the selectedcourseno to set
	 */
