 ...
 * @version 1.0
 */
@Controller("trainInfoDetailAction")
@Scope("prototype")
public class TrainInfoDetailAction extends BaseAction implements ModelDriven<TrainInfoDetail> {
    ...
    private TrainInfoDetail info;

    @Override
    public TrainInfoDetail getModel() {
        if (info == null) {
            info = new TrainInfoDetail();
        }
        return info;
    }
    ...
    @Resource
    private TrainInfoDetailService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<TrainInfoDetail> page = service.getList(new Page<TrainInfoDetail>(req));
            //
    ...
    /**
     * @Title: getTrainInfoDetail
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getTrainInfoDetail() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            TrainInfoDetail info = service.getTrainInfoDetailByid(loadId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生培训情况明细表导入模板.xlsx".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "TrainInfoDetail.xlsx";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<TrainInfoDetail>(req));
    ...
    public TrainInfoDetail getInfo() {
        return info;
    }

    public void setInfo(TrainInfoDetail info) {
        this.info = info;
    }
