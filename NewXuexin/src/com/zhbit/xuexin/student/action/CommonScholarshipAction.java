 ...
 * @date 创建时间：2016-3-15 上午12:34:54
 * @version 1.0
 */
@Controller("commonScholarshipAction")
@Scope("prototype")
public class CommonScholarshipAction extends BaseAction {

    /**
     * @Fields serialVersionUID : TODO(简单说明是做什么的。)
     */
    private static final long serialVersionUID = -8069137343386813403L;
    ...
    @Resource
    private CommonScholarshipService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<CommonScholarship> page = service.getList(new Page<CommonScholarship>(req));
            //
    ...
    public void save() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            int state = service.save(studentno, stuname, major, rewardname, academicyear, term, memo, getSessionUser()
                    .getUserId());
            //
    ...
    public void update() {
        ...
        try {
        	//许彩开 2017.04.19
        	CommonScholarship oldInfo=service.getCommonScholarshipById(id);
        	this.stuname=oldInfo.getStuname();
            service.update(id, studentno, stuname, major, rewardname, academicyear, term, memo);
     ...
     * @return void
     */
    public void getCommonScholarship() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            CommonScholarship info = service.getCommonScholarshipById(id);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
        ...
        try {
            ...
            if (excel != null) {
                ...
                if (".xlsx".equalsIgnoreCase(suffix) || ".xls".equalsIgnoreCase(suffix)) {
                    ...
                    count = service.importFile(excel, user);
                    if (count[0] < 0) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("普通奖学金导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "commonscholarship.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<CommonScholarship>(req));
