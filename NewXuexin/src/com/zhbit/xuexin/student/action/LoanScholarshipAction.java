 ...
 *@date 创建时间：2016-3-21 上午12:23:20
 *@version 1.0
 */
@Controller("loanScholarshipAction")
@Scope("prototype")
public class LoanScholarshipAction extends BaseAction implements ModelDriven<LoanScholarship>{

    /** 
    * @Fields serialVersionUID : TODO(简单说明是做什么的。) 
    */ 
    private static final long serialVersionUID = 4179478747791759849L;

    private LoanScholarship info;
    @Override
    public LoanScholarship getModel() {
        if (info == null) {
            info = new LoanScholarship();
        }
        return info;
    }
    ...
    @Resource
    private LoanScholarshipService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<LoanScholarship> page = service.getList(new Page<LoanScholarship>(req));
            //
    ...
    public void save() {
        ...
        try {
            ...
            int state = service.save(info, getSessionUser().getUserId(),repaydateStr,modifydateStr);
            if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
				map.put("status", "SnoOrSnameIsError");
			} else if (state == 3) {
				map.put("status", Const.DATA_EXIST);
			} else if (state == 4) {
				map.put("status", "IdcardnoIsError");
			} else {
				map.put("status", Const.CODE_FAIL);
			}
        }catch(Exception err) {
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            LoanScholarship oldInfo = service.getLoanScholarshipByid(info.getId());
            int state=service.update(info,oldInfo,repaydateStr,modifydateStr);
            if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
				map.put("status", "SnoOrSnameIsError");
    ...
    /**
     * @Title: getLoanScholarship
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getLoanScholarship() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            LoanScholarship info = service.getLoanScholarshipByid(lsId);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生贷款表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "LoanScholarship.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<LoanScholarship>(req));
    ...
    public LoanScholarship getInfo() {
        return info;
    }

    public void setInfo(LoanScholarship info) {
        this.info = info;
    }
