 ...
 * @date 创建时间：2016-3-11 下午1:09:45
 * @version 1.0
 */
@Controller("politicalStatusAction")
@Scope("prototype")
public class PoliticalStatusAction extends BaseAction {

    /**
     * @Fields serialVersionUID : TODO(简单说明是做什么的。)
     */
    private static final long serialVersionUID = 8792501401899134780L;
    ...
    @Resource
    private PoliticalStatusService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<PoliticalStatus> page = service.getList(new Page<PoliticalStatus>(req));
            //
    ...
    public void save() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            int state = service.save(studentno, stuname, joindate, politicalstatus, memo, getSessionUser().getUserId());
            //
            if (state == 1) {
                map.put("status", Const.CODE_SUCCESS);
            }
    ...
    public void update() {
        Map<String, Object> map = Const.getJsonMap();
        try {
           int result = service.update(id, studentno, stuname, joindate, politicalstatus, memo);
            if(result==1)
            	map.put("status", Const.CODE_SUCCESS);
            else
            	map.put("status", Const.CODE_FAIL);
     ...
     * @return void
     */
    public void getPoliticalStatus() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            PoliticalStatus info = service.getPoliticalStatusById(id);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学生党团关系表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "politicalstatus.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<PoliticalStatus>(req));
