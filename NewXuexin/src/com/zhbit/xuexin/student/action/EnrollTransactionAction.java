 ...
 * @version 1.0
 */
@Controller("enrollTransactionAction")
@Scope("prototype")
public class EnrollTransactionAction extends BaseAction implements ModelDriven<EnrollTransaction> {
    ...
    private EnrollTransaction info;

    @Override
    public EnrollTransaction getModel() {
        if (info == null) {
            info = new EnrollTransaction();
        }
        return info;
    }
    ...
    @Resource
    private EnrollTransactionService service;
    ...
    public void getList() {
        ...
        try {
            ...
            Page<EnrollTransaction> page = service.getList(new Page<EnrollTransaction>(req));
    ...
    public void save() {
		...
		try {
			...
			int state = service.save(info, getSessionUser().getUserId(),tanDate, hanDate, canDate);
			if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
...
//            System.out.println("---------------"+info.getId());
            EnrollTransaction oldInfo = service.getEnrollTransactionByid(info.getId());
            int state =service.update(info, oldInfo, tanDate, hanDate, canDate);
			if (state == 1) {
				map.put("status", Const.CODE_SUCCESS);
			} else if (state == 2) {
				map.put("status", "SnoOrSnameIsError");
    ...
    /**
     * @Title: getEnrollTransaction
     * @Description: TODO(获取对象信息。)
     * @author liangriyu
     * @date 2016-3-11 上午2:34:01
     * @return void
     */
    public void getEnrollTransaction() {
        Map<String, Object> map = Const.getJsonMap();
        try {
            EnrollTransaction info = service.getEnrollTransactionByid(eid);
            map.put("info", info);
            map.put("status", Const.CODE_SUCCESS);
        }
        catch(Exception err) {
    ...
    public void exportTemplate() {
        ...
        try {
            ...
            getResponse().setHeader("Content-Disposition",
                    "attachment;filename=" + new String("学籍异动表导入模板.xls".getBytes("GBK"), "ISO8859-1"));

            String templatePath = ApplicationUtil.getInstance().getProjectPath() + File.separator + "template"
                    + File.separator + "EnrollTransaction.xls";
    ...
    public void exportExcelList() {
        try {
            HttpServletRequest req = getRequest();
            ExportExcelVO vo = service.exportExcelList(new Page<EnrollTransaction>(req));
    ...
    public EnrollTransaction getInfo() {
        return info;
    }

    public void setInfo(EnrollTransaction info) {
        this.info = info;
    }
