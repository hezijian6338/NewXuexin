...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">学生学习情况管理&nbsp;&gt;&gt;&nbsp;辅导记录管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'guiddate',width:120,formatter:function(r){return formatShortDate(r);}">辅导时间</th>
						<th data-options="field:'guidcontent',width:200">辅导内容</th>
						<th data-options="field:'guidaddress',width:120">辅导地点</th>
						<th data-options="field:'counselor',width:100">辅导导师</th>
						<th data-options="field:'docpath',width:120">电子文档</th>
						<th data-options="field:'mediapath',width:120">多媒体文件路径</th>
					</tr>
				</thead>
			</table>
		</div>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'stu/guidanceInfoAction_getList.action',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增辅导记录信息",750,400,"stu/guidanceInfoAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_add","修改辅导记录信息",750,400,"stu/guidanceInfoAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post("stu/guidanceInfoAction_delete.action", {
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入辅导记录信息",450,200,"stu/guidanceInfoAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		queryForm.action="stu/guidanceInfoAction_exportExcelList.action";
		queryForm.submit();
	}
