...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">学生学习情况管理&nbsp;&gt;&gt;&nbsp;学生职务信息管理</div>
			</div>
			...
			<form id="queryForm" method="post">
				...
				<table>
					<tr>
						...
						<td>职务: <input type="text" class="l_textbox" id="duty"
							name="duty" value="">
						</td>
						<td>年级: <select id="grade" name="grade"
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'telno',width:120">手机</th>
						<th data-options="field:'shorttelno',width:100">短号</th>
						<th data-options="field:'address',width:120">宿舍</th>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'tea/studentDutysAction_getList.action',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学生职务信息",750,400,"tea/studentDutysAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学生职务信息",750,400,"tea/studentDutysAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post("tea/studentDutysAction_delete.action", {
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入学生职务信息",450,200,"tea/studentDutysAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		queryForm.action="tea/studentDutysAction_exportExcelList.action";
		queryForm.submit();
	}
