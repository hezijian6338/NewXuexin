...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100,border:false">
			<div class="nav">
				<div class="nav_left">学生学习情况管理&nbsp;&gt;&gt;&nbsp;学生培训情况明细管理</div>
			</div>
			...
			<table id="dg" style="height:200px" data-options="border:false">
				<thead>
					<tr>
						...
						<th data-options="field:'trainsresult',width:120">培训结果</th>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'tea/trainInfoDetailAction_getList.action',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学生培训情况明细",750,400,"tea/trainInfoDetailAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学生培训情况明细",750,400,"tea/trainInfoDetailAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post("tea/trainInfoDetailAction_delete.action", {
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入学生培训情况明细",450,200,"tea/trainInfoDetailAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		queryForm.action="tea/trainInfoDetailAction_exportExcelList.action";
		queryForm.submit();
	}
