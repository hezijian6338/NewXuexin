...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">基础数据&nbsp;&gt;&gt;&nbsp;学生学习经历管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'duration',width:80">学习期间</th>
						<th data-options="field:'schoolname',width:50">学校名称</th>
						<th data-options="field:'duty',width:50">担任职务</th>
						<th data-options="field:'witness',width:120">证明人</th>
					</tr>
				</thead>
			</table>
		</div>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : '${isStu?"stu/learningExperienceAction_getSelf.action":"stu/learningExperienceAction_getList.action"}',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学习经历信息",750,400,"stu/learningExperienceAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_add","修改学习经历信息",750,400,"stu/learningExperienceAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post('${isStu?"stu/learningExperienceAction_deleteSelf.action":"stu/learningExperienceAction_delete.action"}', { 
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入学生信息",450,200,"stu/learningExperienceAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/learningExperienceAction_exportExcelList.action";
		queryForm.submit();
	}
