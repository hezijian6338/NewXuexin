...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">基础数据&nbsp;&gt;&gt;&nbsp;学生家庭信息管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'name',width:80">家庭成员姓名</th>
						<th data-options="field:'call',width:50">称呼</th>
						<th data-options="field:'politicalstatus',width:50">政治面貌</th>
						<th data-options="field:'jobduty',width:120">职务</th>
						<th data-options="field:'telno',width:120">电话号码</th>
						<th data-options="field:'company',width:200">家庭成员单位</th>
						<th data-options="field:'companyaddress',width:250">单位地址</th>
						<th data-options="field:'postcode',width:150">邮政编码</th>
					</tr>
				</thead>
			</table>
		</div>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : '${isStu?"stu/familyInfoAction_getSelf.action":"stu/familyInfoAction_getList.action"}',
			lines : true,
			animate : true,
			nowrap : false,
			fit : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学生家庭信息",750,400,"stu/familyInfoAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学生家庭信息",750,400,"stu/familyInfoAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post('${isStu?"stu/familyInfoAction_deleteSelfFamilyInfo.action":"stu/familyInfoAction_delete.action"}', { 
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入学生家庭信息",450,200,"stu/familyInfoAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/familyInfoAction_exportExcelList.action";
		queryForm.submit();
	}
