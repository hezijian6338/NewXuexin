...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">学生毕业信息&nbsp;&gt;&gt;&nbsp;学生档案邮寄管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'emsno',width:70">EMS编号</th>
						<th data-options="field:'schoolno',width:80">北理工编号</th>
						<th data-options="field:'studentno',width:100">学号</th>
						<th data-options="field:'stuname',width:80">姓名</th>
						<th
							data-options="field:'sex',width:40,formatter:function(r){return showSex(r)}">性别</th>
						<th data-options="field:'major',width:150">专业</th>
						<th data-options="field:'dispatchtype',width:100">派遣性质</th>
						<th data-options="field:'chargeunit',width:250">主管单位</th>
						<th data-options="field:'mailno',width:120">邮件号*</th>
						<th data-options="field:'memo',width:140">备注</th>
					</tr>
				</thead>
			</table>
		</div>
...
<script type="text/javascript">
	$(function() {
		...
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'stu/postInfoAction_getList.action',
			lines : true,
			animate : true,
			nowrap : true,
			fit : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学生档案邮寄信息",750,400,"stu/postInfoAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学生档案邮寄信息",750,400,"stu/postInfoAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post('stu/postInfoAction_delete.action', {
						ids : ids
	...
	function importStuents() {
		initWindow("dg_import","学生档案邮寄信息",450,200,"stu/postInfoAction_viewImport.action"); 
	}
	
	//导出
	function exportStudents(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/postInfoAction_exportExcelList.action";
		queryForm.submit();
	}
