...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">学生学习情况管理&nbsp;&gt;&gt;&nbsp;学生学科竞赛获奖管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'rewardname',width:120">奖励名称</th>
						<th data-options="field:'rewardlevel',width:120">获奖级别</th>
						<th data-options="field:'rewardgrade',width:100">获奖等级</th>
						<th data-options="field:'grantunits',width:100">授予单位</th>
						<th data-options="field:'rewardproject',width:120">获奖项目</th>
						<th data-options="field:'guidteacher',width:150">指导老师</th>
						<th
							data-options="field:'rewarddate',width:110,formatter:function(r){return formatShortDate(r)}">获奖时间</th>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'stu/subjectContestAction_getList.action',
			lines : true,
			animate : true,
			nowrap : false,
			fit : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学科竞赛信息",750,400,"stu/subjectContestAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学科竞赛信息",750,400,"stu/subjectContestAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post('stu/subjectContestAction_delete.action', {
						ids : ids
	...
	function importStuents() {
		initWindow("dg_import","导入学科竞赛信息",450,200,"stu/subjectContestAction_viewImport.action"); 
	}
	
	//导出
	function exportStudents(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/subjectContestAction_exportExcelList.action";
		queryForm.submit();
	}
	...
	var arr = $('#dg').datagrid('getSelections');
		...
		initWindow("dg_upload", "上传学生毕业证书信息附件", 450, 200,
				"stu/subjectContestAction_viewUpload.action?id="+id);
	}
	...
	var arr = $('#dg').datagrid('getSelections');
		...
		var id = arr[0].id;
		location.href = "stu/subjectContestAction_downloadFile.action?sid="+id;
	}
