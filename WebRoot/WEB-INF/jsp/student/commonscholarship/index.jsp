...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">奖助贷管理&nbsp;&gt;&gt;&nbsp;普通奖学金</div>
			</div>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'stu/commonScholarshipAction_getList.action',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增普通奖学金信息",750,400,"stu/commonScholarshipAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改普通奖学金信息",750,400,"stu/commonScholarshipAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post("stu/commonScholarshipAction_delete.action", {
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入普通奖学金名单",450,200,"stu/commonScholarshipAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		queryForm.action="stu/commonScholarshipAction_exportExcelList.action";
		queryForm.submit();
	}
