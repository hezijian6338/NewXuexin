...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">基础数据&nbsp;&gt;&gt;&nbsp;学生高考成绩管理</div>
			</div>
			...
			<form id="queryForm" method="post">
				...
				<table>
					<tr>
						...
						<td>准考证号: <input type="text" class="l_textbox" id="zkhNo"
							name="zkhNo" value="">
						</td>
						<td>身份证号: <input type="text" class="l_textbox" id="idcardno"
							name="idcardno" value="">
						</td>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'zkhNo',width:150">准考证号</th>
						<th data-options="field:'idcardno',width:180">身份证号</th>
...
<script type="text/javascript">
	$(function() {
		...
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : '${isStu?"stu/nceeScoreAction_getSelf.action":"stu/nceeScoreAction_getList.action"}',
			lines : true,
			animate : true,
			nowrap : true,
			fit : true,
	...
	function onAdd() {
		initWindow("dg_add","新增学生高考入学成绩信息",750,400,"stu/nceeScoreAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改学生高考入学成绩信息",750,400,"stu/nceeScoreAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post('${isStu?"stu/nceeScoreAction_deleteSelf.action":"stu/nceeScoreAction_delete.action"}', { 
						ids : ids
	...
	function importStuents() {
		initWindow("dg_import","学生高考入学成绩信息",450,200,"stu/nceeScoreAction_viewImport.action"); 
	}
	
	//导出
	function exportStudents(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/nceeScoreAction_exportExcelList.action";
		queryForm.submit();
	}
