...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'north',height:100">
			<div class="nav">
				<div class="nav_left">学生学习情况管理&nbsp;&gt;&gt;&nbsp;导学内容管理</div>
			</div>
			...
			<table id="dg" style="height:200px">
				<thead>
					<tr>
						...
						<th data-options="field:'guiddate',width:120,formatter:function(r){return formatShortDate(r)}">导学时间</th>
						<th data-options="field:'guidcontent',width:120">导学内容</th>
						<th data-options="field:'guidaddress',width:100">导学地点</th>
						<th data-options="field:'docpath',width:100">电子文档</th>
...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			idField : 'id',
			loadMsg : '数据加载中请稍后……',
			url : 'stu/learningGuidInfoAction_getList.action',
			lines : true,
			animate : true,
	...
	function onAdd() {
		initWindow("dg_add","新增导学内容",750,400,"stu/learningGuidInfoAction_viewAdd.action");
	}
	...
	function onUpdate() {
		...
		var id = arr[0].id;
		initWindow("dg_edit","修改导学内容",750,400,"stu/learningGuidInfoAction_viewEdit.action?id="+id);
	}
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm("提示信息", "确认删除?", function(r) {
				if (r) {
					...
					$.post("stu/learningGuidInfoAction_delete.action", {
						ids : ids
	...
	function importExcel() {
		initWindow("dg_import","导入导学内容",450,200,"stu/learningGuidInfoAction_viewImport.action"); 
	}
	
	//导出
	function exportExcel(){
		//location.href="${ctx}/channeldistr!exportExcelList.action";
		queryForm.action="stu/learningGuidInfoAction_exportExcelList.action";
		queryForm.submit();
	}
