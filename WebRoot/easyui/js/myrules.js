function easyui_form_validator(){
	$
	.extend(
			$.fn.validatebox.defaults.rules,
			{
				...
				academicyear : {
					validator : function(value, param) {
						if (/^20[0-9]{2}-20[0-9]{2}$/.test(value)) {
							var per = parseInt(value.substr(0,4));
							var next = parseInt(value.substr(5));
							return (next-per) == 1;
						} else {
							return false;
						}
					},
					message : '请输入正确的学年,例如"2016-2017"'
				},
				term : {
					validator : function(value, param) {
				...
				finalscore:{
					validator : function(value, param) {
						var reg = /^(优秀|良好|中等|及格|不及格|旷考|100|0|[1-9]\d?|([A-E](\+|-)?))$/
